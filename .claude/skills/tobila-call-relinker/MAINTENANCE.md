...
├── SendTobilaDrafts.gs ← 確認通知メールの下書きを送信するGoogle Apps Script
├── MAINTENANCE.md      ← 本書
└── README.md           ← 概要
```
...
| 症状 | 確認すること |
|---|---|
| 朝の報告メールが来ない | (1)Routineが下書きを作れているか（claude.ai/code/scheduled の実行履歴→セッションログ。コネクタ認証切れが典型）。(2)下書きはあるが送信されない→ SendTobilaDrafts.gs のトリガーが動いているか・DRY_RUNがfalseか・件名に `[tobila-relinker]` が付いているかを確認 |
| メールが下書きのまま送信されない | SendTobilaDrafts.gs を実行するGoogleアカウントが、下書きを作成するアカウント（Gmailコネクタ接続先）と同一か確認。件名マーカー `[tobila-relinker]` の不一致、CONFIG.DRY_RUN=true、トリガー未作成も原因 |
