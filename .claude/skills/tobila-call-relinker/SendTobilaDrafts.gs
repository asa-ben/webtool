...
var CONFIG = {
  ...
  // 安全策：作成からこの時間（時間単位）以内の下書きのみ送信する。
  // この値は「GASが連続して送信に成功しなかった場合に許容する最大ギャップ」に相当する。
  // 短すぎると、GASが1日失敗したり土日・祝日を挟んだ際に下書きを取りこぼす（期限切れで
  // 永久にスキップされ続ける）。送信済み下書きはDraftsから消えるため二重送信は起きず、
  // このチェックの本来の目的は「人が手で残している下書きの誤送信防止」だけなので、長めでよい。
  // 96時間＝土日＋祝日（3連休）を挟んでも翌営業日に確実に拾い直せる。
  // GW・お盆・年末年始など4日超の連休後は sendTobilaDraftsNow()（経過時間無視）を一度実行する。
  MAX_AGE_HOURS: 96,
